...
const ProductTabs = ({ product }) => {
  ...
  return (
    <div>
      <Tab.Group>
        <Tab.List className="border-b py-[2rem] border-gray-200 box-border grid grid-cols-2">
          {tabs.map((tab, i) => {
            return (
              <Tab
                key={i}
                className={({ selected }) =>
                  clsx(
                    "text-center futuraMedium uppercase text-small-regular pb-2 -mb-px border-b border-gray-200 transition-color duration-150 ease-in-out",
                    {
